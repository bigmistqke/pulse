...
- **(worth) Speculative writes invalidate only direct subscribers — anything further downstream goes stale.** `writeSpeculative` marks dirty via `edgesToFire(node, scope)`, which walks `node.subs` — the edges whose `source` is the written node — and stops there. It never walks on from those targets. Meanwhile `readValue` returns `slot.cached` whenever it isn't `undefined`, without checking whether the slot's own deps have since been dirtied. So in a chain `A → B → C` (C's recipe reads B, B's recipe reads A), a speculative write to `A` drops `B.cached` but leaves `C.cached` intact, and the next `readValue(C)` returns a value derived from the superseded `A`. Repro (inside one `action()`): read `C` → `3`; `writeValue(a, 10)`; read `C` again → **`3`, expected `21`**. The one-hop control is correct (reading `B` directly gives `20`), so the edge index and `chainMatch` are sound — only the propagation depth is wrong. Fix is either a transitive dirty-walk from the written node, or a version/epoch check in `readValue` that recomputes when any dep's slot is dirty. Note this is a glitch in the speculation engine specifically; the committed path goes through r3, which propagates topologically.
  Source: mycelium v3 design session — pre-registered prediction from a read of `src/scope.ts`, confirmed by probe test against `401de25`.

- **(worth) `undefined` doubles as the dirty sentinel, so a computed that legitimately evaluates to `undefined` is never cached.** `readValue` recomputes when `slot.cached === undefined && slot.recipe !== undefined`, and `writeSpeculative` marks a slot dirty by assigning `edge.target.cached = undefined`. Since `Slot.cached` is typed `T | undefined`, a recipe whose value genuinely *is* `undefined` is indistinguishable from a dirty slot: it re-runs on every read (measured: **4 recipe invocations across 3 reads** inside one `action()`), memoisation never engages, and any side effect in the recipe fires per-read. The code cannot separate "no value yet" from "the value is `undefined`". Fix wants a distinct sentinel — `const DIRTY = Symbol()`, `cached: T | typeof DIRTY` — which makes the ambiguous state unrepresentable rather than merely tested against.
  Source: as above.
...
- **(later) `commit()` does not fire scope cleanups; `discard()` does.** `discard()` runs `scope.cleanups` LIFO and clears the array. `commit()` promotes, closes edges, sets `status`, and never touches `cleanups` — so a cleanup registered in a scope that commits is silently dropped, and the array is left populated. `closeScopeEdges`'s docblock says *"Does NOT promote or fire cleanups — later plans add those around this"*, so this may well be deliberate staging rather than a defect — but nothing in the code or the types records which, and the two closing paths are asymmetric on identical state. Worth an explicit decision: either `commit()` fires them symmetrically with `discard()`, or a comment on `commit()` states that commit-path cleanups are the caller's problem.
  Source: as above.
