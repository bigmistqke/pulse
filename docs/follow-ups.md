...
- **(worth) `computed(() => fetchSomething(deps).then(...))` infinite-loops.** When a single-stage sync computed's body returns a Promise whose IDENTITY changes across body re-runs (e.g. via a `.then(...)` chain that creates a new wrapper each call), the framework loops: body returns Promise → computed value = Promise → `.then` registers kick → Promise settles → kick fires → body re-runs → returns NEW Promise → ... forever. Demos hit this when the user "naturally" writes a computed that derives a Promise from a reactive dep. **Workaround:** wrap in `signal` + an `effect` that calls `setSignal(s, fetchSomething(deps))` — the effect re-runs only on actual dep change, not on settle; write-back doesn't re-trigger the effect (it doesn't read its own output). **Real fix:** either (a) cache the Promise externally so the computed body returns stable identity, (b) require generator-form computeds (`yield* read(promise)`) for async derivations (the WeakMap key still needs stable Promise identity, so doesn't fully fix it), or (c) the framework detects "body re-ran with identical structural inputs but different Promise identity" and treats it as a no-op. Document the constraint loudly in `CONTEXT.md` `Computed` entry until a real fix lands.
  Source: Plan 4 demos (examples/pokemon Playwright tests; signal+effect pattern works, computed pattern infinite-loops).
