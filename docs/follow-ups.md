...
- **(later) `effect()` stages with explicit commit terminator.** Pulse's `computed(...stages)` accepts a variadic stage pipeline; an `effect([...stages], commit)` form would generalize the same machinery to effects, with `commit` being the side-effect terminator that the `<Loading>` boundary can defer alongside DOM bindings. This closes the "plain effects don't gate" wrinkle in Plan B by giving effects a named callback to hand to the boundary. Designed but deferred to a Plan C — not bundled with Plan B to keep its scope tight.
