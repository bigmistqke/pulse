...
- `signal.ts` setter, when the written value is a promise (currently `track(value)`) → wrap in an `Awaitable`, **seeded with the prior value read from the current node**: `toAwaitable(p, readValue(node) instanceof Awaitable ? readValue(node).value : readValue(node))`. Note this is a *distinct* mechanism from `computed.ts` — the setter has no `lastResolvedValue` closure to borrow; the prior must come from the node's current value. (This also makes the new SWR *strictly better* than the old `latest()`, which misses SWR when `latest(s)` is never called between a settle and the next write — its `lastResolved` cache goes unpopulated. The eager `Awaitable.value` removes that gap.)
- `computed.ts` when `runStage` returns a pending outcome (`outcome.promise`) and on the `NotReadyYet` catch path → wrap in an `Awaitable`, seeded from the stage's `lastResolvedValue` (which the settle handlers already maintain eagerly).
- One helper (`toAwaitable(promise, prior?)`) covers both boundaries; first-load has no prior (`.value` undefined until settle). Each new `Awaitable` seeds `.value` from its predecessor, so an `Awaitable1 → Awaitable2 → …` refetch chain propagates the last settled value correctly.

**3. `.value` subsumes `latest`; `committed` is new.** `latest(` has **no `src/` callers** — only `test/async.test.ts` and `test/integration-async.test.ts` (and the public export). So the readiness `latest(s)` maps to `s().value`, and — because it is public — `latest` can be **kept for backward-compat** rather than removed. Not a naïve `return s().value` (that breaks for a plain-value signal, where `s()` is a bare `T` and `(0).value` is `undefined`): the compat shim is `const v = s(); return v instanceof Awaitable ? v.value : v`. The *isolation* read `committed(s)` is a **new** verb (from the scope overlay, `src/scope.ts`) with no existing callers to migrate. So: `latest` → alias of `.value` (readiness, compat-preserving); `committed` → new (isolation).
