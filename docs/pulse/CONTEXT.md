...
- **`get(node)`** — the unified read walk. A **sync** node reads as bare `T`; an **async** node (`Node<Promise<U>>`) reads as a plain `Promise<U>` — *uniformly*, staying a `Promise` after settling rather than flipping to a bare `U`. Resolve it through verbs: [`latest(node)`](#async) (synchronous peek, `U | undefined` while pending — the stale-while-revalidate read), `yield* read(node)` (await, for composition inside a stage/action), and [`use(node)`](#walks) (suspend). The promise's status, value, and stale prior live in one WeakMap keyed on the promise, not on the object. Per [`async-reads-and-coordination.md`](./async-reads-and-coordination.md) and [ADR 0012](../adr/0012-weakmap-backed-promise-read-model.md), which supersede [ADR 0002](../adr/0002-pending-model.md)'s write-back. Used by computeds, effects, and the JSX binding alike.
- **`peek(node)`** — non-tracking read. Currently under review for removal; see [`questions.md`](./questions.md).
- **`committed(node)`** — returns the last committed value, bypassing the active speculation's slot (the *isolation* axis). Named `committed` (not `latest`) per [`async-reads-and-coordination.md`](./async-reads-and-coordination.md#lexicon-deltas), to avoid clashing with the shipped readiness-axis `latest` (the stale read; see Async) and with Solid's `latest`; it aligns with `commit` / `discard` / `onCommit` / `onDiscard`.
...
- **Async read (`Promise<T>`).** An async node reads as a plain `Promise<T>` — not a subclass, and it carries no extra fields. Its status (pending / fulfilled / rejected), resolved value, rejection reason, and stale-while-revalidate prior live in one WeakMap keyed on the promise. Read it through verbs, never off the object: `latest` (the stale value, `T | undefined` while pending), `use` (suspend), `isPending`, `yield* read` (await, for composition inside a stage/action), and `settled` (wait-for-all). Disambiguation: `latest(node) === undefined` with `isPending(node) === false` is a genuine `undefined`; `isPending(node) === true` is not-ready. Per [`async-reads-and-coordination.md`](./async-reads-and-coordination.md#part-1--the-read-model-an-async-read-is-a-plain-promise) and [ADR 0012](../adr/0012-weakmap-backed-promise-read-model.md).
- **`latest(node)`.** The readiness-axis stale read: the last resolved value, returned without suspending, staying the prior value while a newer promise is pending (`undefined` until the first settle). Distinct from `committed` (the isolation-axis read).
- **`settled([a, b, …])`.** A *wait-for-all* combinator — the plural form of `yield* read(node)`. Awaits every input and resolves to the tuple of values once all have settled (≈ `Promise.all` over the reads). The read-side coordination barrier for a shared consumer: keep the last coherent frame (via stale-while-revalidate), swap atomically when every input is ready. Per [`async-reads-and-coordination.md`](./async-reads-and-coordination.md#part-2--the-coordination-barrier-settled-on-stale-while-revalidate).
