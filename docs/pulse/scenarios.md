...
- **W3.** Write before the signal has ever been read — seeding from a cache during startup. Tests: for a synchronous derivation, the update function receives the value it already resolved to at creation, not `undefined`. For an asynchronous one, the write cancels the startup fetch — since a single-stage pipeline's tail is the only stage, cancelling leaves it clean, so the abandoned fetch never lands and the seeded value is not later replaced by a real load. ✓ (Walked, then corrected twice during implementation. First: the design assumed a derivation is lazy and would first run on the first read; it is eager and has already run and suspended by the time the write lands, so `undefined` means nothing has *resolved*, not that nothing has run. Second, caught only in the final whole-branch review: the scenario's own `Tests:` clause claimed the seeded value stays visible "until the request lands," describing stale-while-revalidate — but the request the write cancels never lands, so there is nothing to revalidate from. See the follow-up this produced in `docs/follow-ups.md`.)
...
- **W5.** Write onto a parked failure. Tests: the failure clears and an `<Errored>` boundary stops showing its fallback. ✓ (Implemented and tested in `test/writable-derived.test.ts`.)
