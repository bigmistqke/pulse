...
**The conceptual overload is named and resolved.** [`concurrent-divergence.md`](./concurrent-divergence.md) unpacks that pulse's `Scope` bundles two jobs — atomic commit boundary *and* isolation context — and shows the 1:1 coupling is a *deliberate* choice, not an accident: isolation is what gives atomicity its bite, so "shared visibility, independent commit" (Solid's lane-merge direction) is nearly unwinnable (the cascade-discard problem). Entanglement therefore reframes into three separable concerns — none of them Solid's auto-merge:

- *Deliberate coupling* → **nested actions** (share fate on purpose; traced in [G2](./scenario-traces.md#g2--nested-actions-and-commit-promotion)).
- *Conflicts within an isolated speculation* → **`onConflict: 'reject'`** (traced in [D1](./scenario-traces.md#d1--read-dependent-write-under-reject)).
- *Coordinating a shared consumer's display across overlapping async edits* → a consumer-side **rendering barrier**, `settled([...])` on stale-while-revalidate — the design that came out of this exploration, in [`async-reads-and-coordination.md`](./async-reads-and-coordination.md).

**Solid's own answer — union-lane merge — is a *trap* for pulse.** It fuses two speculations that touch shared state in order to reconcile them; but pulse's multi-slot-per-Node ([Q1](#q1--fall-through-and-edge-policy) / [Q9](#q9--read-populated-vs-write-populated-slots-do-they-differ-structurally)) means overlapping speculations never *collide* at a node — each gets its own slot — so there is nothing to merge. The "peek at each other" Solid's merge produces is an emergent symptom of its single-lane-per-computation storage, not a feature to port. The genuinely-interleaved concurrent-async case is traced in [CC1](./scenario-traces.md#cc1--two-concurrent-async-speculations-interleaved-resumptions): the single-body invariants compose across true interleaving, resting on the async driver restoring the ambient scope per resume.

**Ship-set (candidates, informed by scenarios not usage):** keep defaults; add `'reject'` (the one new *conflict* primitive, traced); document the CRDT-signal-value pattern for class B; ship no `'rebase'` / merge-callbacks / auto-merge. The isolate-by-default decision is recorded as [ADR 0009](../adr/0009-isolate-speculations-by-default.md). Separately, the *rendering*-coordination want (a shared consumer showing overlapping async edits without a torn frame) is answered by the consumer-side `settled([...])` barrier on stale-while-revalidate — see [`async-reads-and-coordination.md`](./async-reads-and-coordination.md), which also refines the async read model to a uniform `Awaitable`. What stays genuinely open is usage-gated: how often class D actually bites, whether class C needs sugar, and the cleanest way to express CRDT-style merge-on-commit signal values.

**Related:** the [four dimensions of transition](../async/CONTEXT.md#the-four-dimensions-of-transition) in the research lexicon (Q15 = Dim 4); [P4](./framings.md#p4--explicit-boundaries-over-implicit-pervasiveness) (explicit boundaries); [P5](./framings.md#p5--compose-dont-proliferate-in-either-direction) (compose, don't proliferate); [Q6](#q6--what-is-a-scope-as-a-value) (the scope-tree mechanism that supports parentless scopes for per-context isolation); [`async-reads-and-coordination.md`](./async-reads-and-coordination.md) (the rendering-coordination barrier + uniform-`Awaitable` read model that came out of this exploration); traces [D1](./scenario-traces.md#d1--read-dependent-write-under-reject) (reject) and [CC1](./scenario-traces.md#cc1--two-concurrent-async-speculations-interleaved-resumptions) (interleaving).
