# Research Process — Async Strategies

> **Renamed from `CONTEXT.md`.** This file is the research *process* doc. The
> name `CONTEXT.md` is now the research *lexicon* — see
> [`CONTEXT.md`](./CONTEXT.md). References to "CONTEXT.md" in older `LOG.md`
> entries or deep-dives that concern sourcing discipline, status indicators, or
> anti-patterns mean *this* file.
...
See the main research log at `./README.md` for the framing, taxonomy table, session chronology, and deep-dive index. Defined terms — the four dimensions, the failure modes, the research vocabulary — live in the lexicon, [`CONTEXT.md`](./CONTEXT.md). This file is the *meta*: how the work is done.
...
The research lexicon — the four dimensions, the failure modes, and precise
vocabulary (Encoding, Axis, System, Verified cell, Concept, …) — lives in
[`CONTEXT.md`](./CONTEXT.md). Terms used precisely in deep-dives and synthesis
docs are defined and maintained there, so the docs reference one source rather
than drifting.
...
- **Don't update PROCESS.md in passing.** Convention changes deserve their own commit and rationale. This file is meta; changing it should be deliberate.
...
- `./CONTEXT.md` — the research lexicon: the four dimensions, the failure modes, and precise vocabulary.
