...
8. **The critical refinement to pulse's design philosophy** (added in merge, connecting to a conversation about pulse's minimum-primitive-exposure direction): the user's framing was *"the only way for complexity to be possible is that it is composed from simple principles."* Svelte 5 is partial evidence for and partial evidence against this. **For:** the user-facing API is genuinely minimal — three primitives (`<svelte:boundary>`, `pending` snippet, `$effect.pending()`) plus optional `fork()` cover the user-facing surface for transitional behavior, no `useTransition` / `useOptimistic` / `useActionState` / lanes-as-public-concept proliferation. **Against:** the implementation that backs those three primitives is ~800 lines of `batch.js` with linked-list batches, time-travel snapshots, rebase, and merge. **The minimum applies to the user-facing API, not the engine.** Concurrent transitions are not free even with simple primitives. If pulse wants `<Loading>` and `use()` to compose from simple primitives that users can themselves implement in userland, the primitives must either (a) not support concurrent transitions, or (b) include enough engine surface to make the multi-batch coordination expressible. Svelte's answer is (b) — the engine is rich, the API is small. The user's articulation can hold as design philosophy provided pulse's research recognizes that "simple primitives" doesn't mean "simple engine," only "simple user-facing surface."
