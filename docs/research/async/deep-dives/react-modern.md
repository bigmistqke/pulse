...
- **`useTransition`'s `isPending` flag is internally implemented as `useOptimistic`.** Below-the-API-surface implementation detail not visible from the react.dev docs. Source: Ricky Hanlon on Syntax.fm episode #943 ([transcript](https://syntax.fm/show/943/modern-react-with-ricky-hanlon-react-core-dev/transcript)) — *"the isPending flag in useTransition is implemented as useOptimistic."* Confirms that React's runtime *fuses* these abstractions internally; at the user level they look like distinct primitives, but mechanically `useOptimistic` is the underlying mechanism that `useTransition`'s pending flag is built on.

- **Suspense fallbacks are throttled at ≥300ms.** Source: same Syntax.fm episode — *"throttling — maintaining fallback visibility for ≥300ms to batch updates."* When a boundary suspends, the fallback isn't always shown immediately; React throttles fallback visibility to reduce DOM layout thrashing and coordinate animations. This is below-the-API-surface behavior not exposed via configuration; readers expecting Suspense to swap fallback ↔ content instantly will observe the throttled behavior in practice.
