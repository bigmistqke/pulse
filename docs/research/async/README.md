...
- **Atomicity granularity for systems with TWO atomicity layers.** effect-ts has STM-level (per-commit) AND fiber-level scope-bounded (per-scope) atomicity, for different concerns (data consistency vs resource lifecycle). A single cell value flattens this. Consider whether the axis should split into "data-atomicity" and "lifecycle-atomicity."
