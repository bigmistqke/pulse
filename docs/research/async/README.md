...
9. **Speculative-state isolation** (added session 9, promoted from candidate; refined session 12 to add Svelte's "unbounded observable batches" value). Distinct from isolation level. Asks "is there an isolated parallel state being built that's invisible until commit, that can be discarded mid-build?" Values (now seven): *none* (direct mutation); *per-action overlay* (explicit user-managed parallel state — `useOptimistic`, Recoil snapshot); *per-write-lane overlay with overlap-merge* (Solid 2.x); *per-transition tree* (React WIP fiber, pulse `<Loading>` gather); *versioned engine, fixed-cardinality observable branches* (Replicache main+sync heads); *versioned engine, unbounded observable batches* (Svelte 5 linked-list of `Batch` objects; `fork()` as flagged subtype); *versioned everywhere* (full MVCC — Postgres, Yjs, event sourcing).
...
| 🟢 | **Xilem + Druid** (Linebender, Rust) ([dive](./deep-dives/xilem-druid.md)) | app state by user convention; framework provides no async-state container; `task`/`worker` views carry only `JoinHandle<()>` as ViewState | last-write-wins with stale-message discard (`MessageResult::Stale` for vanished views; generation counter on `xilem_web` await-shaped views) | **structural via tokio `JoinHandle::abort()` on view `teardown`** — cancellation by view-tree shape (no user-held abort handle) | Rust `Future` + tokio (`spawn`); `MessageProxy<M>` = typed-message + id-path; tasks-as-views (`task`/`worker`) with `fork` for invisible lifecycle mounting | none / shared mutable through single-threaded event loop | per-message (each `on_event`/`on_response` callback runs to completion; no transaction grouping) | mostly user-side (loading state, enums, branching) + framework-side message routing & JoinHandle abort | coarse view-diff (re-execute `view()`, structural diff against previous view tree); no fine-grained read-tracking; **IC framing's payoff is typed addresses, not dataflow** (resolves session-10 SAC-silent-on-async gap) |
...
| 🟢 | **Svelte 5 runes** (`experimental.async` from 5.36) ([dive](./deep-dives/svelte-5.md)) | regular `Source` cell populated from deferred handler; gated by `<svelte:boundary>` or `Batch`; no separate "async resource" type | **per-derived cancel-previous-on-rerun** (`OBSOLETE` reject of older deferreds; "all in-flight" while boundary still pending; "only previous single run" once rendered) + **per-batch whole-batch merge** on source-set intersection | two channels: `OBSOLETE` (per-derived rejection, swallowed by handler) and `STALE_REACTION` (per-effect abort); `getAbortSignal` for cooperative user opt-in; no automatic fetch abort | plain `Promise<V>`; awaited inside `async_effect`; no `Resource`/observable/typed-Effect | per-batch `batch_values` snapshot + `current`/`previous` maps; forks are deeper-isolated (`source.v` not written) | per-`<svelte:boundary>` for first render; per-`Batch` for subsequent updates; per-explicit-`fork()` for opt-in speculation | framework/compiler (`experimental.async` flag); user writes `await` and gets coherence; only user-visible controls are `<svelte:boundary>`, `$effect.pending()`, `settled()`, `fork()` | fused; `await` participates in dep-tracking via compiler-emitted `capture`/`save`; downstream reads use same `get()` path as sync state |
...
| 🟢 | **Xilem + Druid** | **none** (no transitions, no optimistic, no overlay; app state is mutated in place; loading state is user-modelled enum) | await-only (`.await`/`join!`/`select!` from tokio; no pipelining; no batching layer above plain futures) |
...
| 🟢 | **Svelte 5 runes** | **versioned engine, unbounded observable batches** (linked-list of `Batch` objects with per-batch `batch_values` snapshot + `current`/`previous` maps; `wv` write-version per source; `fork()` is a flagged subtype of same machinery — sits between Replicache's fixed-cardinality and full MVCC) | await-only with implicit ordering (sequential `$derived(await …)` decls run serially; framework warns on `await_waterfall`; independent `await` in markup run concurrently) |
...
- [ ] **Vue 3 Composition API + `<Suspense>`** (comparison dive; Svelte 5 done separately)
- [x] **Svelte 5 runes** with `experimental.async` ([dive](./deep-dives/svelte-5.md), session 12) — async derivations via `await` inside `$derived`, `<svelte:boundary>` for gather-on-commit, `fork()` for opt-in speculation. **Falsified hypothesis** that Svelte handles only Dim 1; it handles Dim 1+2+4 with serious machinery (~800 lines of `batch.js`). Surfaced 7th value on speculative-state-isolation axis: "versioned engine, unbounded observable batches." **Critical refinement to pulse's "minimum primitives" design philosophy:** the minimum applies to user-facing API (3 primitives), NOT to the engine.
- [x] **Xilem + Druid (Linebender, Rust)** ([dive](./deep-dives/xilem-druid.md), session 11) — async coordination in IC-influenced Rust UI. Tasks-as-views with structural cancellation via `JoinHandle::abort()` on view teardown. **The IC framing's payoff is typed addresses, not dataflow** — resolves session-10's SAC-silent-on-async gap. No Suspense/transition/optimistic analog (user-modelled enums + `OneOf*` rendering); Rust's exhaustive matching makes the manual approach survivable in a way it isn't in JS. **Loading-boundary primitive is more valuable in JS than in Rust** (genuine cross-language design finding).
