import { computed, For, isLoading, Loading, render, Show, signal, use } from 'pulse'
	...
	return (
		<Loading initial={<div class="detail-spinner">loading details…</div>}>
			<div class="details">
				...
				<div class="meta">
					<div class="types">
						<For each={use(pokemon).types}>{t => <span class="type">{t.type.name}</span>}</For>
					</div>
					<table class="stats">
...
function PokemonRow(props: { ref: PokemonRef }) {
	...
	return (
		<li class:expanded={isExpanded()}>
			<button
				class="name"
				on:click={() => setExpanded(c => (c === props.ref.name ? null : props.ref.name))}
			>
				{props.ref.name}
			</button>
			<Show when={isExpanded()}>{() => <PokemonDetails name={props.ref.name} />}</Show>
		</li>
	)
}
			...
			<Loading initial={<div class="spinner">loading…</div>}>
				<div class="loaded">
					...
					<nav class="paging">
						...
						<span class:loading={isLoading()}>page {use(page) + 1}</span>
