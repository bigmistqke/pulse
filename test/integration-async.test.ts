...
import { effect, peek, signal, use } from '../src/index'
...
test('peek gives stale-while-revalidate across a re-fetch', async () => {
  ...
  await tick()
  expect(peek(data)).toBe(1)

  setData(Promise.resolve(2))
  expect(peek(data)).toBe(1) // stale value held while the new promise is pending
  await tick()
  expect(peek(data)).toBe(2)
})
